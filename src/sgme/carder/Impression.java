...
import javax.swing.*;
 ...
 public final class Impression implements Printable 
 { 
     public Impression(JPanel panel) 
     { 
         documentTitle = ""; 
         this.panel = panel; 
         //this.panel.setSize((int)pageFormat.getWidth(), (int)pageFormat.getHeight());
         initPrintablePanel();
     } 
   ...
   public int getPageHeight()
   {
    ...
    { 
        ...
        paper.setImageableArea(0 /*paper.getImageableX() - (double)(margin / 2)*/, paper.getImageableY(), 
               /* paper.getImageableWidth() + (double)(margin / 2)*/ 0, paper.getImageableHeight());
        pageFormat.setPaper(paper);
    } 
    public void setTBMargins(int margin) 
    { 
        Paper paper = pageFormat.getPaper(); 
        paper.setImageableArea(paper.getImageableX(), paper.getImageableY() - (double)(margin / 2),
                paper.getImageableWidth(), paper.getImageableHeight() + (double)(margin / 2));
        pageFormat.setPaper(paper);
    }
    ...
    { 
        ...
        if(wrapComponent) totalNumPages = taillePages.size();
        else if(fitIntoPage) totalNumPages = 1; 
        double scaleX = (largeurPage / largeurDocu); 
        double scaleY = hauteurPage / hauteurDocu;        
        if(pageIndex >= totalNumPages) return 1; 
        ...
        { 
            if(pageIndex > 0) g2d.translate(0.0D, -((Double)taillePages.get(pageIndex - 1)).doubleValue());
        } 
        else 
        {
        ...
        { 
            ...
            g2d.setColor(Color.WHITE); g2d.fillRect(0, (int)hauteurBlanc, (int)largeurPage+25, (int)hauteurBlanc + (int)hauteurPage);
        } 
        if(wrapComponent) 
        { 
            if(pageIndex > 0) g2d.translate(0.0D, ((Double)taillePages.get(pageIndex - 1)).doubleValue());
        ...
        g2d.drawString(documentTitle + " - [" + (pageIndex + 1) + "/" + totalNumPages + "]", 0, (int)pf.getImageableHeight() );
        return 0; 
    } 
    public void print() 
    { 
        ...
        { 
            ...
            {
                if(wrapComponent) calculatePages(); 
                Paper paper = pageFormat.getPaper(); 
                Paper save = pageFormat.getPaper();
                paper.setImageableArea(paper.getImageableX(), paper.getImageableY(), paper.getWidth(), paper.getHeight() - paper.getImageableY()); 
                pageFormat.setPaper(paper);
                printJob.setPrintable(this, pageFormat);
                printJob.print(); pageFormat.setPaper(save);
            } 
        } 
...
{ 
    ...
    { 
        ...
        if(panel.getLayout() instanceof GridBagLayout) 
            gridBagInsets = ((GridBagLayout)panel.getLayout()).getConstraints(panel.getComponent(i)).insets.bottom 
                + ((GridBagLayout)panel.getLayout()).getConstraints(panel.getComponent(i)).insets.top; 
        double hauteurComponent = panel.getComponent(i).getSize().getHeight() + (double)gridBagInsets;
        if(hauteurComponent > hauteurPage) 
        {
            wrapComponent = false; return; 
        } 
